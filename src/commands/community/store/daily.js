...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("daily")
    .setDescription("Get the daily claim for your role"),
  async execute(interaction) {
    ...
    const timeRestriction = 24 * 60 * 60 * 1000; // 24 hours in milliseconds
    ...
    if (lastClaim && currentTime - lastClaim.date < timeRestriction) {
      ...
      const cooldownEmbed = new EmbedBuilder()
        .setColor("Red")
        .setDescription(
          `:hourglass_flowing_sand: ${member}, you can only claim once every 24 hours. Please try again later.`
        );
